create table    jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_mm_2 as
select * from (
...
s.bp_sk, s.bp_par_sk,  Coalesce(seg.segn_sk,s.dr_seg_sk) as dr_seg_sk, max(plan_dtl_ct) as plan_dtl_ct , s.segn_sk, s.cycl_time_id, s.scen_id, s.inrt_dt, s.inrt_by, 
s.modf_dt, s.modf_by 
...
create table    jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_lym_2 as
select * from (
select sorg_sk,pset_sk,sls_wrkr_sk,strt_cald_dt_sk,bp_sk,bp_par_sk,dr_seg_sk,plan_dtl_ct,segn_sk, dsegn.cycl_time_id, dsegn.scen_id, dsegn.inrt_dt, dsegn.inrt_by, 
dsegn.modf_dt, dsegn.modf_by ,rank() over (partition by bp_par_sk,bp_sk,pset_sk 
order by prio.segn_prio ) as  rank  from 
...
delete from jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_lym_2 where pset_sk in (3076,3078);
delete from jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_mm_2 where pset_sk in (1728,3080,1798);
...
alter table jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_lym_2 drop column rank;
alter table jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_mm_2 drop column rank;


delete from JP2A_CDW.XREF_RPT_TRGT_DR where pset_sk in (3431,3429) and strt_cald_dt_sk NOT IN (20201201);
 INSERT INTO JP2A_CDW.XREF_RPT_TRGT_DR SELECT * FROM jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_mm_2 WHERE strt_cald_dt_sk NOT IN (20201201);
 INSERT INTO JP2A_CDW.XREF_RPT_TRGT_DR SELECT * from  jp_sandbox_ops.rpt_trgt_dr_temp_2_hist_hema_mm_2 WHERE strt_cald_dt_sk NOT IN (20201201);
